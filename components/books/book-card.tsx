    ...
    return (
        <div
            ...
            className={`book relative mx-auto h-auto sm:h-[420px] bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 flex cursor-pointer group hover:shadow-2xl hover:shadow-emerald-500/10 transition-all duration-500 border border-gray-200/50 dark:border-gray-700/50 rounded-2xl overflow-hidden ${isExpanded
                    ? "w-full max-w-full sm:max-w-[580px] sm:col-span-2"
                    : "w-full max-w-[280px] flex-col items-center p-4"
                }`}
            style={isExpanded ? { gridColumn: "span 2" } : {}}
        >
            {/* Discount Badge */}
            <div
                className={`absolute top-3 bg-gradient-to-r from-red-500 to-red-600 text-white text-xs font-bold px-2 py-1 rounded-full z-10 shadow-lg ${isExpanded ? "left-3" : "right-3"
                    }`}
            >
                {book.discount}% OFF
            </div>
            ...
            {isExpanded ? (
                ...
                <div className="flex flex-col sm:flex-row w-full h-full">
                    {/* Left Side - Image */}
                    <div className="w-full sm:w-[200px] h-[240px] sm:h-full flex-shrink-0 relative">
                    ...
                    <div className="flex-1 p-4 flex flex-col justify-between">
                        ...
                        <div className="space-y-3">
                            <div>
                                <h3 className="title font-bold text-lg text-gray-900 dark:text-gray-50 leading-tight mb-1">
                                    {book.title}
                                </h3>
                                <p className="author font-medium text-sm text-gray-600 dark:text-gray-300">by {formatAuthors(book.authors)}</p>
                            </div>
                        ...
                        <div className="space-y-3">
                            <div className="price-section">
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="current-price font-bold text-lg text-emerald-600 dark:text-emerald-400">
                                        ₹ {discountedPrice}
                                    </span>
                                </div>
                            ...
                            <div className="flex gap-2">
                                ...
                                <button
                                    ...
                                    className="bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 px-3 py-2 rounded-full font-medium transition-all duration-300 text-xs"
                                >
                                    Collapse
                                </button>
                            </div>
                ...
                <>
                    ...
                    <div className="img-container relative w-[180px] h-[240px] mb-3 transform group-hover:scale-105 group-hover:-rotate-1 transition-all duration-500 rounded-xl overflow-hidden shadow-lg group-hover:shadow-xl">
                    ...
                    <div className="info w-full text-center flex flex-col items-center gap-2 flex-1">
                        <div className="title-author flex flex-col items-center mb-2">
                            ...
                            <p className="author font-medium text-sm text-gray-600 dark:text-gray-300 px-2">by {formatAuthors(book.authors)}</p>
                        </div>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 rounded-2xl flex flex-col items-center justify-end pb-8">
                        <div className="transform translate-y-8 group-hover:translate-y-0 transition-transform duration-500 flex flex-col gap-3">
                            <button
                                ...
                                className="bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white px-6 sm:px-8 py-2.5 sm:py-3 rounded-full font-semibold shadow-lg hover:shadow-xl transition-all duration-300 text-sm"
                            >
                            ...
                            <button
                                onClick={handleAddToCart}
                                className={`bg-white/90 hover:bg-white text-gray-900 px-6 sm:px-8 py-2 sm:py-2.5 rounded-full font-medium shadow-lg hover:shadow-xl transition-all duration-300 text-sm backdrop-blur-sm relative overflow-hidden ${isAddingToCart || isAdded ? "pointer-events-none" : ""}`}
                            >
                                <div
                                    className={`flex items-center justify-center w-full transition-all duration-500 ${isAddingToCart ? "translate-x-[120%]" : isAdded ? "translate-x-0" : "translate-x-0"}`}
                                >
                                    <ShoppingCart className={`w-4 h-4 ${isAddingToCart ? "mr-0" : "mr-2 inline-block"}`} />
                                    <span>{isAdded ? "Added" : "Add to Cart"}</span>
                                </div>
                                {isAddingToCart && (
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <ShoppingCart className="w-4 h-4 animate-[cartRide_1s_ease-in-out]" />
                                    </div>
                                )}
                                {isAdded && <Check className="w-4 h-4 mr-2 inline-block" />}
                            </button>
                        </div>
                    </div>
